    ...
    def expand(self, alpha=float('-inf'), beta=float('inf')):
        ...
        if self.is_max:
            ...
            for i in possible_moves:
                new_state = self.new_state_g(i) # Create the new state for the children node
                child_node = Tree_A(new_state, move=i, parent=self, is_max=False)  # Create a new child with of class tree_A with the new state as parameter
                self.add_child(i, child_node) # The child node is aded to the list of child node of the current node being treated
                eval_value = child_node.expand(alpha, beta) # Determine the value of the node
                max_eval = max(max_eval, eval_value) # Chose the max value between current max eval and the evaluation value of the child node that as been processed
                alpha = max(alpha, eval_value) #Chose the max value between current alpha val and the evaluation value of the child node that as been processed
            ...
            for i in possible_moves:
                new_state = self.new_state_g(i) # Create the new state for the children node
                child_node = Tree_A(new_state, move=i, parent=self, is_max=True) # Create a new child with of class tree_A with the new state as parameter
                self.add_child(i, child_node) # The child node is aded to the list of child node of the current node being treated
                eval_value = child_node.expand(alpha, beta) # Determine the value of the node
                min_eval = min(min_eval, eval_value) # Chose the min value between current min eval and the evaluation value of the child node that as been processed
                beta = min(beta, eval_value) #Chose the min value between current beta val and the evaluation value of the child node that as been processed
