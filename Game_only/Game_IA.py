...
def selct(number, score, bank, turn, algo, first, sec, order):
    global main_w
    if turn == 1:
        score, bank = 0, 0
    ...
    if dividers:
        ...
        if first.startswith("AI"):
            ...
            number //= best_divider
            end_time = time.time()
            tk.Label(main_w, text=f"{first} chose divisor: {best_divider}, New value: {number}, Execution Time: {end_time - start_time:.6f}").grid(row=2, column=3)
            tk.Button(main_w, text="Next Turn", command=lambda: selct(number, score, bank, turn + 1, algo, sec, first, order)).grid(row=3, column=3)
        else:
            tk.Label(main_w, text=f"The number is: {number}").grid(row=3, column=3)
            
            for col, i in enumerate(dividers):
                tk.Button(main_w, text=str(i), command=lambda i=i: selct(number // i, score, bank, turn + 1, algo, sec, first, order)).grid(row=2, column=col)
    else:
        main_w.destroy()
        end(number, score, bank, turn, order)

def end(number, score, bank, turn, order):
    global end_w
    if end_w:
        end_w.destroy()
    ...
    fichier =  open("data.txt", "a")
    fichier.write(f"Final Number: {number}, Turns: {turn - 1}, Score: {score}, Bank: {bank}, Final Score: {Final}, Winner: {winner}")
    fichier.close()
...
def first_p(number, score, bank, turn, algo, p1, p2):
    ...
    for i, player in enumerate([p1, p2]):
        tk.Button(main_w, text=player, command=lambda player=player: selct(number, score, bank, turn, algo, player, p1 if player == p2 else p2, [player, p1 if player == p2 else p2])).grid(row=2 + i, column=0)
